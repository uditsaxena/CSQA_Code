...
gold_file = "model_softmax_kvmem_validtrim_unfilt_newversion/test_output_" + sys.argv[2] + "_" + sys.argv[
    1] + "/gold_resp.txt"  # sys.argv[1]
pred_file = "model_softmax_decoder_newversion/test_output_" + sys.argv[2] + "_" + sys.argv[
    1] + "/pred_sent.txt"  # sys.argv[2]
ent_file = "model_softmax_kvmem_validtrim_unfilt_newversion/test_output_" + sys.argv[2] + "_" + sys.argv[
    1] + "/top20_ent_from_mem.txt"
...
for goldline, predline, entline in zip(goldlines, predlines, entlines):
    ...
    '''
		...
		if gold_number2 is None:
			gold_number2 = "None"
		'''
    gold_parsed = str(gold_entity1) + '(' + str(gold_number1) + ') ' + str(gold_entity2) + '(' + str(
        gold_number2) + ') '
    pred_parsed = str(pred_entity1) + '(' + str(pred_number1) + ') ' + str(pred_entity2) + '(' + str(
        pred_number2) + ') '
    # if acc-acc_old>0:
