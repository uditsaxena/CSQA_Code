...
def feeding_dict(model, mem_size, inputs_w2v, inputs_kb, orig_target, target, decoder_target, text_weights, mem_weights,
                 decoder_inputs, sources, rel, key_target, feed_prev, ent_embedding, rel_embedding):
    feed_dict = {}
...
def get_test_op(sess, model, batch_dict, step, ent_embedding):
    memory_size, test_batch_enc_w2v, test_batch_enc_kb, batch_target, batch_orig_target, batch_response, batch_orig_response, batch_text_weight, batch_mem_weight, batch_decoder_input, batch_sources, batch_rel, batch_key_target, batch_active_set, _ = get_batch_data(
        param['max_len'], param['max_utter'], param['memory_size'], param['gold_target_size'], param['batch_size'],
        batch_dict)
    feed_dict = feeding_dict(model, memory_size, test_batch_enc_w2v, test_batch_enc_kb, batch_orig_target, batch_target,
                             batch_response, batch_text_weight, batch_mem_weight, batch_decoder_input, batch_sources,
                             batch_rel, batch_key_target, True, ent_embedding, rel_embedding)
    dec_op, loss, loss_decoder, loss_kvmem = sess.run([logits, losses, losses_decoder, losses_kvmem],
                                                      feed_dict=feed_dict)
...
def run_training(param):
    def get_train_loss(model, batch_dict, feed_prev, step, ent_embedding, rel_embedding, type_of_loss):
        ...
        memory_size, train_batch_enc_w2v, train_batch_enc_kb, batch_target, batch_orig_target, batch_response, batch_orig_response, batch_text_weight, batch_mem_weight, batch_decoder_input, batch_sources, batch_rel, batch_key_target, batch_active_set, _ = get_batch_data(
            param['max_len'], param['max_utter'], param['memory_size'], param['gold_target_size'], param['batch_size'],
            batch_dict, overriding_memory)
        feed_dict = feeding_dict(model, memory_size, train_batch_enc_w2v, train_batch_enc_kb, batch_orig_target,
                                 batch_target, batch_response, batch_text_weight, batch_mem_weight, batch_decoder_input,
                                 batch_sources, batch_rel, batch_key_target, feed_prev, ent_embedding, rel_embedding)
        if type_of_loss == "decoder":
    ...
    def get_valid_loss(model, batch_dict, ent_embedding, rel_embedding, id_entity_map, type_of_loss):
        if type_of_loss == "decoder":
            ...
            overriding_memory = None
        memory_size, val_batch_enc_w2v, val_batch_enc_kb, batch_target, batch_orig_target, batch_response, batch_orig_response, batch_text_weight, batch_mem_weight, batch_decoder_input, batch_sources, batch_rel, batch_key_target, batch_active_set, _ = get_batch_data(
            param['max_len'], param['max_utter'], param['memory_size'], param['gold_target_size'], param['batch_size'],
            batch_dict, overriding_memory)
        feed_dict = feeding_dict(model, memory_size, val_batch_enc_w2v, val_batch_enc_kb, batch_orig_target,
                                 batch_target, batch_response, batch_text_weight, batch_mem_weight, batch_decoder_input,
                                 batch_sources, batch_rel, batch_key_target, True, ent_embedding, rel_embedding)
        if type_of_loss == "decoder":
            ...
            gold_entity_ids = [[id_entity_map[gold_entity_ids[i][j]] for j in range(gold_entity_ids.shape[1])] for i in
                               range(gold_entity_ids.shape[0])]
    ...
    def perform_training(model, batch_dict, feed_prev, step, ent_embedding, rel_embedding, type_of_loss):
        if type_of_loss == "decoder":
            batch_train_loss, dec_op = get_train_loss(model, batch_dict, feed_prev, step, ent_embedding, rel_embedding,
                                                      type_of_loss)
        elif type_of_loss == "kvmem":
            batch_train_loss, output_prob = get_train_loss(model, batch_dict, feed_prev, step, ent_embedding,
                                                           rel_embedding, type_of_loss)
    ...
    def perform_evaluation(model, batch_dict, batch_target_word_ids, batch_text_targets, epoch, step, vocab,
                           ent_embedding, rel_embedding, id_entity_map, type_of_loss, wikidata_id_name_map):
        if type_of_loss == "decoder":
            batch_valid_loss, valid_op, batch_orig_response = get_valid_loss(model, batch_dict, ent_embedding,
                                                                             rel_embedding, id_entity_map, type_of_loss)
            batch_predicted_sentence, prob_predicted_words, prob_true_words = get_predicted_sentence(valid_op,
                                                                                                     batch_target_word_ids,
                                                                                                     vocab)
            print_pred_true_op_decoder(batch_predicted_sentence, prob_predicted_words, prob_true_words,
                                       batch_text_targets, step, epoch, batch_valid_loss)
        elif type_of_loss == "kvmem":
            batch_valid_loss, prob_mem_entries, prob_mem_scores, gold_entity_ids, gold_orig_response = get_valid_loss(
                model, batch_dict, ent_embedding, rel_embedding, id_entity_map, type_of_loss)
            print_pred_true_op_kvmem(prob_mem_entries, prob_mem_scores, gold_entity_ids, gold_orig_response, step,
                                     epoch, batch_valid_loss, wikidata_id_name_map)
        sum_batch_loss = get_sum_batch_loss(batch_valid_loss)
        return sum_batch_loss

    def evaluate(model, epoch, step, valid_data, valid_text_targets, vocab, ent_embedding, rel_embedding, id_entity_map,
                 loss_type, wikidata_id_name_map):
        ...
        for i in range(n_batches):
            ...
            sum_batch_loss = perform_evaluation(model, batch_dict, batch_target_word_ids, batch_target_sentences, epoch,
                                                step, vocab, ent_embedding, rel_embedding, id_entity_map, loss_type,
                                                wikidata_id_name_map)
    ...
    def print_pred_true_op_kvmem(prob_memory_entities, prob_memory_scores, gold_entity_ids, gold_orig_response, step,
                                 epoch, batch_valid_loss, wikidata_id_name_map):
        for i in random.sample(xrange(len(gold_orig_response)), 5):
            ...
            x = [unidecode.unidecode(mem_entry) for mem_entry in prob_memory_entities[i] if
            ...
            print [wikidata_id_name_map[id] for id in prob_memory_entities[i] if id in wikidata_id_name_map if
                   wikidata_id_name_map[id] not in ['<pad_kb>', '<nkb>', '<unk>']][:5]
    ...
    if param['type_of_loss'] == 'decoder':
        allowed_states = ['Verification (Boolean) (All)', 'Quantitative Reasoning (Count) (All)',
    ...
    with tf.Graph().as_default():
	    ...
	    '''
                ...
                train_data.sort(
                    key=lambda x: len(x[6].split("|")))  # filter(lambda a: a!= kb_pad_idx, x[6].split("|"))))
                print 'sorted training data by memory size'
                n_batches = int(math.ceil(float(len(train_data)) / float(param['batch_size'])))
                print 'number of batches ', n_batches, 'len train data ', len(train_data), 'batch size', param[
                    'batch_size']
                sys.stdout.flush()
                for i in range(n_batches):
                    ...
                    sum_batch_loss = perform_training(model, train_batch_dict, feed_prev, overall_step_count, ent_embed,
                                                      rel_embed, type_of_loss)
                    ...
                    if overall_step_count > 0 and overall_step_count % param['valid_freq'] == 0:
                        overall_avg_valid_loss = evaluate(model, epoch, i, valid_data, valid_text_targets,
                                                          response_vocab, ent_embed, rel_embed, id_entity_map,
                                                          type_of_loss, wikidata_id_name_map)
                        print (
                        'Epoch %d Step %d ... overall avg valid loss= %.6f ' % (epoch, i, overall_avg_valid_loss))
                        sys.stdout.flush()
            ...
            if last_overall_avg_train_loss is not None and overall_avg_train_loss > last_overall_avg_train_loss:
                ...
                if diff > param['train_loss_incremenet_tolerance']:
                    print 'WARNING: training loss (%.6f) has increased by %.6f since last epoch, has exceed tolerance of %f ' % (
                    overall_avg_train_loss, diff, param['train_loss_incremenet_tolerance'])
                else:
                    print 'WARNING: training loss (%.6f) has increased by %.6f since last epoch, but still within tolerance of %f ' % (
                    overall_avg_train_loss, diff, param['train_loss_incremenet_tolerance'])
...
def main():
    ...
    print param
    if isinstance(param['train_data_file'], list) and isinstance(param['valid_data_file'], list) and all(
            [os.path.exists(x) for x in param['train_data_file']]) and all(
            [os.path.exists(x) for x in param['valid_data_file']]):
        print 'dictionary already exists'
        sys.stdout.flush()
    elif isinstance(param['train_data_file'], str) and isinstance(param['valid_data_file'], str) and os.path.exists(
            param['train_data_file']) and os.path.exists(
            param['valid_data_file']):  # and os.path.exists(param['test_data_file']):
        print 'dictionary already exists'
        sys.stdout.flush()
    else:
